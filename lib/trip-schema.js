...
/* Les six volets ne tiennent pas dans un seul appel : la requête dépasse la
   durée maximale d'une fonction serverless et se fait couper avant la fin,
   sans jamais aboutir — et chaque reprise la relance en entier. On les
   sépare donc en deux passes, écrites l'une après l'autre et fusionnées.

   La coupure suit le sens : d'abord ce qui conditionne le départ — entrer
   dans le pays, s'y soigner, y être joignable — puis ce qui se prépare
   ensuite. */
export const PREP_A_SCHEMA = obj({
  formalities: obj({
    ...
    documents: arr(
      ...
      "3 à 5 documents réellement exigés ou utiles pour un ressortissant français sur cette destination, mineurs compris s'il y en a"
    ),
  ...
  health: obj({
    ...
    kit: arr(str("Un article de la trousse, motivé par cette destination"), "4 à 6 articles"),
    emergency: arr(
      obj({
        label: str("Le service : urgences, police, ambassade de France…"),
        number: str("Le numéro tel qu'on le compose sur place"),
      }),
      "3 à 4 numéros réels dans ce pays"
    ),
    facilities: arr(
      obj({
      ...
      "1 à 2 établissements réels, proches des étapes"
    ),
    safety: arr(
      obj({
      ...
      "2 à 3 points réellement spécifiques"
    ),
  }),
  ...
  connectivity: obj({
    ...
    esim: arr(
      ...
      "2 à 3 offres réelles couvrant la durée du séjour"
    ),
    plug: obj({
...
export const PREP_B_SCHEMA = obj({
  money: obj({
    currency: obj({
  ...
  packing: arr(
    ...
    "4 à 5 groupes, dictés par la météo, la saison, la durée, les activités prévues, la présence d'enfants et les usages locaux"
  ),
  ...
  bookings: arr(
    ...
    "4 à 6 réservations réellement nécessaires à cet itinéraire, ordonnées par urgence"
  ),

  /* L'écran n'affiche que le mode conseillé : demander la comparaison
     complète produisait trois options que personne ne lisait. */
  transport: obj({
    verdict: str("Comment se déplacer sur place, en une phrase"),
    mode: str("Le mode conseillé : « Transports urbains », « Voiture de location », « Train »…"),
    priceEur: num("Coût indicatif par jour, en euros"),
  ...
  watchouts: arr(
    ...
    "2 à 3 pièges réels dans cet itinéraire : marges trop courtes, journées trop chargées, fermetures…"
  ),
});
