/* Recherche mondiale : aéroports et destinations.
   ...
   Format d'une entrée : [IATA, nom, ville, pays ISO, rang, alias?]
...
function build(raw) {
  const airports = raw.map(([code, name, city, cc, rank, alias]) => {
    const country = countryName(cc);
    return { code, name, city, cc, country, rank, hay: norm(`${code} ${name} ${city} ${country} ${alias || ""}`) };
  });
