...
import { nightsOf, addUsage, prepComplete } from "./claude";
...
export async function appendDays(id, batch, total, usage) {
  ...
  /* Refaire une journée d un voyage déjà prêt ne doit pas le renvoyer en
     composition : le statut se déduit de ce qui est écrit, pas de l appel. */
  const ready = days.length >= total && prepComplete(trip.practical);
  await query(
    `UPDATE trips SET days = $1, usage_json = $2, status = $3, error = NULL, updated_at = $4 WHERE id = $5`,
    [JSON.stringify(days), usage ? JSON.stringify(usage) : null, ready ? "ready" : "building", now(), id]
  );
