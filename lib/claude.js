...
async function run({ system, prompt, schema, searchUses, maxTokens }) {
  ...
  for (let attempt = 0; attempt <= DEGRADATIONS.length; attempt++) {
    ...
    try {
      ...
      if (err?.status !== 400 || attempt === DEGRADATIONS.length) throw err;
...
function briefText(brief) {
  ...
  const includes = Object.entries({
    ...
    .filter(([k]) => brief.include?.[k])
    .map(([, v]) => v)
    .join(", ");

  return [
    `Destination souhaitée : ${brief.dest}`,
...
Recherche d'abord : la saison réelle sur ces dates, les compagnies qui desservent cet axe, les quartiers où loger, les ordres de prix constatés. Puis découpe le séjour en étapes cohérentes avec le temps disponible — mieux vaut deux étapes vécues que cinq traversées.

Si la destination demandée est vague (« du soleil », « la montagne »), choisis la destination précise la plus pertinente pour ces dates et ce profil, et dis-le dans le résumé.`;
