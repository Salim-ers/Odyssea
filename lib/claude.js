...
import { PLAN_SCHEMA, DAYS_SCHEMA, PREP_SCHEMA } from "./trip-schema";
...
export const EXPECTED_OUTPUT = { plan: 3800, days: 750, practical: 5200 };
...
export async function generatePractical(brief, plan, days, onEvent) {
...
${programme}
${knownSources(plan)}
Prépare maintenant le départ, en six volets : les formalités, la santé et la sécurité, la connexion, l'argent, la valise, et ce qu'il faut réserver.

Tout doit être propre à ce pays, à ces dates et à cet équipage. Ce qui vaudrait pour n'importe quelle destination n'a pas sa place ici : ni « prévoyez une trousse à pharmacie », ni « vérifiez la validité de votre passeport » sans dire combien de mois sont exigés.
...
Pièges : relis le programme et cherche ce qui coince vraiment — une marge trop courte avant un vol, deux journées de marche consécutives, un lieu fermé le jour où il est placé, un trajet sous-estimé. Chiffre-les. Si l'itinéraire est sain, dis-le plutôt que d'inventer un problème.`;
  ...
  const { text, degraded, usage } = await run({
    ...
    /* Six volets détaillés : la marge de 16 000 tronquait la valise. */
    maxTokens: 22000,
    onEvent,
  });
  return { practical: parseJson(text), degraded, usage };
}
