...
export async function forecast({ lat, lon, start, end }) {
  const url = `${FORECAST}?latitude=${lat}&longitude=${lon}&daily=${DAILY}&timezone=auto&start_date=${start}&end_date=${end}`;
...
export async function normals({ lat, lon, start, end }) {
  ...
  const runs = await Promise.all(
    years.map((y) =>
      fetchJson(
        `${ARCHIVE}?latitude=${lat}&longitude=${lon}&daily=${DAILY}&timezone=auto` +
          `&start_date=${addYears(start, -y)}&end_date=${addYears(end, -y)}`
      ).catch(() => null)
    )
  );
  ...
  for (const run of runs) {
    ...
    d.time.forEach((date, i) => {
      ...
      const b = buckets.get(key) || { tmax: [], tmin: [], rain: [], codes: [] };
      ...
      if (d.precipitation_probability_max?.[i] != null) b.rain.push(d.precipitation_probability_max[i]);
  ...
  for (let d = new Date(start + "T12:00:00"); iso(d) <= end; d.setDate(d.getDate() + 1)) {
    ...
    if (!b || !b.tmax.length) {
      out.push({ date, kind: "unknown", code: null, tmax: null, tmin: null, rain: null });
      continue;
    }
    ...
    out.push({
      ...
      rain: b.rain.length ? Math.round(mean(b.rain)) : null,
    });
  }
  return out;
}
